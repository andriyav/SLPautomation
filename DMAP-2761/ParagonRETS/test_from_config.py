        ...
        for source in sources:
            ...
            for clas in range(1, classes):
                with self.subTest(source=source):
                    try:
                        metadata_checker(source, "lease_price", 6, clas, "L_AskingPrice")
                        metadata_checker(source, "lease_price", 6, clas, "L_Class")
                        metadata_checker(source, "lease_price", 6, clas, "L_SaleRent")
                        try:
                        ...
                        wrong_mapped.append(source + f" class {clas} metadata wrong")
                        print(f"Error in test for source {source} class {clas}: {str(e)} metadata wrong")
